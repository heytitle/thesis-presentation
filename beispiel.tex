...
\begin{frame}
	...
	\only<1>{
		Two classes of signal \{-1,1\}
			\vspace*{\fill}
			 \begin{figure}[h]
				\includegraphics [width=0.5\textwidth]{figures/esn_toyexample}
			\end{figure}
			\vspace*{\fill}
	}
	\only<2>{
			\vspace*{\fill}
			 \begin{figure}[h]
				\includegraphics [width=\textwidth]{figures/esn_toyexample2}
			\end{figure}
			\vspace*{\fill}
	}
...
%	\begin{enumerate}
		...
		\only<2-3>{- Google's Neural Machine Translation System \citep{DBLP:journals/corr/WuSCLNMKCGMKSJL16}.
			\only<2>{
				\begin{figure}[h]
			...
			\only<3>{
				\begin{figure}[h]
					\includegraphics [width=0.6\textwidth]{figures/gnmt_attention}
				\end{figure}
			}
